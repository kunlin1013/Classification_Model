...
if __name__ == '__main__':
    ...
    if not os.path.exists("save_weights_v3"):
        os.makedirs("save_weights_v3")
    else:
        rmtree("save_weights_v3")
        os.makedirs("save_weights_v3")
    ...
    model = mobilenet_v3_large(input_shape=(224,224,3), nclass=5)
    model.summary()
    ...
    for epoch in range(EPOCHS):
        ...
        if val_loss.result() < best_test_loss:
            ...
            model_savepath = fr'save_weights_v3\model_weights_{epoch+1}_{val_loss.result():.3f}.h5'
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig('accuracy_history_v3.png')
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('loss_history_v3.png')
