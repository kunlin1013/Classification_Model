...
def correct_pad(input_size: Union[int, tuple], kernel_size: int):
    """
    When the convolution stride is 2, calculate an appropriate padding for filling.
...
def _se_block(inputs, filters, prefix, se_ratio=1 / 4.):
    ...
    filters: channel
    prefix: prefix of the layer name
    se_ratio: 1/4 of the number of nodes in the first fully connected layer
    '''
    ...
    # fc1
    x = layers.Conv2D(filters=_make_divisible(filters * se_ratio), kernel_size=1, padding='same',  name=prefix + 'squeeze_excite/Conv')(x)
    x = layers.ReLU(name=prefix + 'squeeze_excite/Relu')(x)

    # fc2
    x = layers.Conv2D(filters=filters, kernel_size=1, padding='same', name=prefix + 'squeeze_excite/Conv_1')(x)
...
def _inverted_res_block(x,
    ...
    if block_id:
        ...
        x = layers.Conv2D(filters=exp_c, kernel_size=1, padding='same', use_bias=False, name=prefix + 'expand')(x)
        x = bn(name=prefix + 'expand/BatchNorm')(x)
        x = act(name=prefix + 'expand/' + act.__name__)(x)

    if stride == 2:
        input_size = (x.shape[1], x.shape[2])  # height, width
        x = layers.ZeroPadding2D(padding=correct_pad(input_size, kernel_size), name=prefix + 'depthwise/pad')(x)

    x = layers.DepthwiseConv2D(kernel_size=kernel_size, strides=stride, padding='same' if stride == 1 else 'valid', use_bias=False, name=prefix + 'depthwise')(x)
    ...
    x = layers.Conv2D(filters=out_c, kernel_size=1, padding='same', use_bias=False, name=prefix + 'project')(x)
...
def mobilenet_v3_large(input_shape=(224,224,3), nclass=1000, alpha=1.0, include_top=True):

    bn = partial(layers.BatchNormalization, epsilon=0.001, momentum=0.99)
    img_input = layers.Input(shape=input_shape)

    x = layers.Conv2D(filters=16, kernel_size=3, strides=(2, 2), padding='same', use_bias=False, name="Conv")(img_input)
    ...
    x = layers.Conv2D(filters=last_c, kernel_size=1, padding='same', use_bias=False, name="Conv_1")(x)
    ...
    if include_top is True:
        ...
        # fc1
        x = layers.Conv2D(filters=last_point_c, kernel_size=1, padding='same', name="Conv_2")(x)
        x = HardSwish(name="Conv_2/HardSwish")(x)

        # fc2
        x = layers.Conv2D(filters=nclass, kernel_size=1, padding='same', name='Logits/Conv2d_1c_1x1')(x)
        x = layers.Flatten()(x)
        y = layers.Softmax(name="Predictions")(x)

    model = Model(img_input, y, name="MobilenetV3large")

    return model


def mobilenet_v3_small(input_shape=(224,224,3), nclass=1000, alpha=1.0, include_top=True):
