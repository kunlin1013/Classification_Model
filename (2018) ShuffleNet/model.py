...
class ConvBNReLU(layers.Layer):
    def __init__(self, filters: int = 1, kernel_size: int = 1, strides: int = 1, padding: str = 'same', **kwargs):
        super(ConvBNReLU, self).__init__(**kwargs)

        self.conv = layers.Conv2D(filters=filters, kernel_size=kernel_size, strides=strides, padding=padding, use_bias=False, name="conv1")
...
class DWConvBN(layers.Layer):
    def __init__(self,
        ...
        self.dw_conv = layers.DepthwiseConv2D(kernel_size=kernel_size, strides=strides, padding=padding, use_bias=False, name="dw1")
        self.bn = layers.BatchNormalization(momentum=0.9, epsilon=1e-5, name="bn")
...
def shufflenet_v2(nclass: int, input_shape: tuple, stages_repeats: list, stages_out_channels: list):
    ...
    x = ConvBNReLU(filters=stages_out_channels[0], kernel_size=3, strides=2, name="conv1")(img_input)

    x = layers.MaxPooling2D(pool_size=(3, 3), strides=2, padding='same', name="maxpool")(x)

    stage_name = ["stage{}".format(i) for i in [2, 3, 4]]
    for name, repeats, output_channels in zip(stage_name, stages_repeats, stages_out_channels[1:]):
    ...
    x = layers.Dense(units=nclass, name="fc")(x)
    x = layers.Softmax()(x)
...
def shufflenet_v2_x1_0(nclass=1000, input_shape=(224, 224, 3)):
    model = shufflenet_v2(nclass=nclass, input_shape=input_shape, stages_repeats=[4, 8, 4], stages_out_channels=[24, 116, 232, 464, 1024])
    return model

def shufflenet_v2_x0_5(nclass=1000, input_shape=(224, 224, 3)):
    model = shufflenet_v2(nclass=nclass, input_shape=input_shape, stages_repeats=[4, 8, 4], stages_out_channels=[24, 48, 96, 192, 1024])
    return model

def shufflenet_v2_x2_0(nclass=1000, input_shape=(224, 224, 3)):
    model = shufflenet_v2(nclass=nclass, input_shape=input_shape, stages_repeats=[4, 8, 4], stages_out_channels=[24, 244, 488, 976, 2048])
    return model
